...
export default function KalkulatorAllegro() {
  ...
  // Pobieranie biblioteki XLSX do eksportu
  useEffect(() => {
    if (!window.XLSX) {
  ...
  const fetchRatesData = useCallback(() => {
    ...
    fetch("https://api.frankfurter.app/latest?base=PLN&symbols=EUR,USD,GBP,CHF,CZK,RON,CNY")
      ...
      .catch(() => {
        setRatesError("Błąd pobierania kursów");
  ...
  return (
    <div style={{
      ...
      <style>{`
        ...
        html, body, #root { margin: 0; padding: 0; width: 100%; min-height: 100vh; background-color: #0f0f13; }
      ...
      <div style={{ textAlign: "center", marginBottom: "2.5rem" }}>
        <div style={{ fontFamily: "'Syne', sans-serif", fontSize: "clamp(1.8rem, 5vw, 3rem)", fontWeight: 800, letterSpacing: "-0.03em", lineHeight: 1, background: "linear-gradient(135deg, #f5a623 0%, #f0623a 100%)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>KALKULATOR</div>
        <div style={{ fontFamily: "'Syne', sans-serif", fontSize: "clamp(1.8rem, 5vw, 3rem)", fontWeight: 800, letterSpacing: "-0.03em", lineHeight: 1, color: "#e8e4d9" }}>MARŻY ALLEGRO</div>
        <div style={{ color: "#5a5a6e", fontSize: "0.78rem", marginTop: "0.5rem", letterSpacing: "0.1em" }}>PROWIZJE · VAT · KONTRAHENCI · AUTOSAVE · EKSPORT XLSX</div>
      </div>

      <div style={{ width: "100%", maxWidth: "680px", display: "flex", flexDirection: "column", gap: "1rem" }}>

        {/* Sekcja Dostawcy (Autosave) */}
        <div style={{ background: "#16161e", borderRadius: "16px", padding: "1.5rem", border: "1px solid #2a2a36" }}>
          <div style={{ fontSize: "0.7rem", letterSpacing: "0.12em", color: "#5a5a6e", marginBottom: "1.2rem" }}>DANE KONTRAHENTA</div>
          <Field label="Nazwa dostawcy / hurtowni" value={supplierName} onChange={setSupplierName} placeholder="np. Centrala Gadżetów Sp. z o.o. (zostanie zapamiętane)" />
        </div>

        {/* Dane Identyfikacyjne Produktu */}
        <div style={{ background: "#16161e", borderRadius: "16px", padding: "1.5rem", border: "1px solid #2a2a36" }}>
          <div style={{ fontSize: "0.7rem", letterSpacing: "0.12em", color: "#5a5a6e", marginBottom: "1.2rem" }}>BIEŻĄCY PRODUKT</div>
        ...
        {/* Inputs card */}
        <div style={{ background: "#16161e", borderRadius: "16px", padding: "1.5rem", border: "1px solid #2a2a36" }}>
          <div style={{ fontSize: "0.7rem", letterSpacing: "0.12em", color: "#5a5a6e", marginBottom: "1.2rem" }}>DANE FINANSOWE</div>

          <Field label="Cena oferty (zł)" value={offerPrice} onChange={setOfferPrice} placeholder="np. 89,99" />

          {/* Koszt zakupu z wyborem waluty */}
          <div style={{ marginBottom: "1rem" }}>
            <label style={{ fontSize: "0.7rem", letterSpacing: "0.1em", color: "#5a5a6e", display: "block", marginBottom: "0.4rem" }}>KOSZT ZAKUPU</label>
            <div style={{ display: "flex", gap: "0.5rem" }}>
              <input
                ...
                style={{ flex: 1, background: "#1e1e28", border: "1px solid #3a3a48", borderRadius: "8px", color: "#e8e4d9", fontSize: "1.05rem", fontFamily: "inherit", padding: "0.6rem 0.9rem" }}
              />
              <select
                value={purchaseCurrency}
                onChange={e => setPurchaseCurrency(e.target.value)}
                style={{ background: "#1e1e28", border: "1px solid #3a3a48", borderRadius: "8px", color: "#f5a623", fontSize: "0.9rem", fontFamily: "inherit", fontWeight: 500, padding: "0.5rem 0.6rem", cursor: "pointer", minWidth: "80px" }}
              >
              ...
              <div style={{ marginTop: "0.45rem", display: "flex", alignItems: "center", gap: "0.5rem", fontSize: "0.7rem" }}>
                {ratesLoading ? (
                  ...
                  <>
                    ...
                    {purchaseCost && costInPLN() && <span style={{ color: "#8a8a9e" }}>= {formatPLN(costInPLN())}</span>}
          ...
          <div style={{ display: "flex", gap: "0.35rem", flexWrap: "wrap", marginBottom: "1.2rem" }}>
            {CURRENCIES.map(c => (
              <button
                ...
                style={{ background: purchaseCurrency === c.code ? "linear-gradient(135deg,#f5a623,#f0623a)" : "#22222e", color: purchaseCurrency === c.code ? "#0f0f13" : "#5a5a6e", borderRadius: "6px", padding: "0.2rem 0.55rem", fontSize: "0.72rem", fontWeight: purchaseCurrency === c.code ? 500 : 400, border: purchaseCurrency === c.code ? "none" : "1px solid #2a2a36" }}
              >
                {c.flag} {c.code}
              </button>
            ))}
            <button
              ...
              title="Odśwież kursy"
              style={{ background: "#22222e", color: "#5a5a6e", borderRadius: "6px", padding: "0.2rem 0.55rem", fontSize: "0.72rem", border: "1px solid #2a2a36", marginLeft: "auto" }}
            >
          ...
          {/* Prowizja */}
          <div style={{ marginBottom: "0.4rem", fontSize: "0.7rem", letterSpacing: "0.12em", color: "#5a5a6e" }}>PROWIZJA ALLEGRO</div>
          <div style={{ display: "flex", alignItems: "center", gap: "0.5rem" }}>
            <input type="text" inputMode="decimal" value={allegro} onChange={e => setAllegro(e.target.value)} style={{ width: "80px", background: "#1e1e28", border: "1px solid #3a3a48", borderRadius: "8px", color: "#f5a623", fontSize: "1.1rem", fontFamily: "inherit", fontWeight: 500, padding: "0.45rem 0.7rem", textAlign: "center" }} />
            <span style={{ color: "#5a5a6e", fontSize: "0.9rem" }}>%</span>
            <div style={{ display: "flex", gap: "0.4rem", marginLeft: "0.4rem", flexWrap: "wrap" }}>
              {[5, 8, 10, 12, 15].map(v => (
                <button key={v} className="pill" onClick={() => setAllegro(String(v))} style={{ background: parseFloat(allegro) === v ? "#f5a623" : "#2a2a36", color: parseFloat(allegro) === v ? "#0f0f13" : "#8a8a9e", border: "none", borderRadius: "6px", padding: "0.25rem 0.6rem", fontSize: "0.75rem", fontFamily: "inherit", cursor: "pointer" }}>{v}%</button>
              ))}
            </div>
          </div>
        </div>
        ...
        {/* VAT + Dostawa */}
        <div style={{ background: "#16161e", borderRadius: "16px", padding: "1.5rem", border: "1px solid #2a2a36", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.5rem" }}>
          <div>
            <div style={{ fontSize: "0.7rem", letterSpacing: "0.12em", color: "#5a5a6e", marginBottom: "0.8rem" }}>STAWKA VAT</div>
            <div style={{ display: "flex", flexDirection: "column", gap: "0.4rem" }}>
              {VAT_OPTIONS.map(v => (
                <button key={v} className="pill" onClick={() => setVat(v)} style={{ background: vat === v ? "linear-gradient(135deg, #f5a623, #f0623a)" : "#2a2a36", color: vat === v ? "#0f0f13" : "#8a8a9e", border: "none", borderRadius: "8px", padding: "0.5rem 0.8rem", fontSize: "0.9rem", fontFamily: "inherit", fontWeight: vat === v ? 500 : 400, cursor: "pointer", textAlign: "left" }}>
                  {v}% VAT {v === 23 && <span style={{ fontSize: "0.65rem", marginLeft: "0.4rem", opacity: 0.7 }}>standard</span>}
                </button>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "0.6rem" }}>
              <div style={{ fontSize: "0.75rem", color: "#8a8a9e", lineHeight: 1.4 }}>Doliczyć 2% kosztu dostawy?</div>
              <div className="pill" onClick={() => setIncludeDelivery(v => !v)} style={{ display: "flex", alignItems: "center", gap: "0.7rem", marginTop: "0.3rem", cursor: "pointer", userSelect: "none" }}>
                <div className="toggle-track" style={{ width: "44px", height: "24px", borderRadius: "12px", background: includeDelivery ? "#f5a623" : "#2a2a36", position: "relative", flexShrink: 0 }}>
                  <div style={{ position: "absolute", top: "3px", left: includeDelivery ? "23px" : "3px", width: "18px", height: "18px", borderRadius: "50%", background: includeDelivery ? "#0f0f13" : "#5a5a6e", transition: "left 0.2s" }} />
                </div>
                <span style={{ fontSize: "0.85rem", color: includeDelivery ? "#f5a623" : "#5a5a6e", fontWeight: includeDelivery ? 500 : 400 }}>{includeDelivery ? "TAK" : "NIE"}</span>
              </div>
              <div style={{ fontSize: "0.7rem", color: "#3a3a4e", lineHeight: 1.4 }}>{includeDelivery ? "2% ceny oferty doliczone" : "Nie wliczany"}</div>
            </div>
          </div>
        </div>
        ...
        {/* Wyniki */}
        {result && (
          <div className="fadeup" style={{ background: "#16161e", borderRadius: "16px", padding: "1.5rem", border: isPositive ? "1px solid #2a4a2e" : isNegative ? "1px solid #4a2a2a" : "1px solid #2a2a36" }}>
            <div style={{ fontSize: "0.7rem", letterSpacing: "0.12em", color: "#5a5a6e", marginBottom: "1rem" }}>WYNIKI KALKULACJI</div>
            ...
            <div style={{ marginBottom: "1rem" }}>
              ...
              {includeDelivery && <ResultRow label="Koszt dostawy 2%" value={"− " + formatPLN(result.deliveryCost)} negative />}
              {result.costPLN && <ResultRow label={`Koszt zakupu (${purchaseCost} ${purchaseCurrency})`} value={formatPLN(result.costPLN)} dimmed />}
            ...
            {result.profit !== undefined && (
              <div style={{ background: isPositive ? "#1a2e1c" : isNegative ? "#2e1a1a" : "#1e1e28", borderRadius: "10px", padding: "1rem 1.2rem", marginTop: "0.5rem", marginBottom: "1rem" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                  <div>
                    <div style={{ fontSize: "0.65rem", letterSpacing: "0.1em", color: "#5a5a6e", marginBottom: "0.3rem" }}>ZYSK / STRATA CZYSTA</div>
                    <div style={{ fontFamily: "'Syne', sans-serif", fontSize: "1.8rem", fontWeight: 800, color: isPositive ? "#4ecb71" : isNegative ? "#e05555" : "#e8e4d9", lineHeight: 1 }}>
                      {isPositive ? "+" : ""}{formatPLN(result.profit)}
                    </div>
                    <div style={{ fontSize: "0.72rem", color: "#5a5a6e", marginTop: "0.3rem" }}>po koszcie zakupu (+2% kapitał: {formatPLN(result.costPlus2)})</div>
                  </div>
                  <div style={{ textAlign: "right" }}>
                    <div style={{ fontSize: "0.65rem", letterSpacing: "0.1em", color: "#5a5a6e", marginBottom: "0.3rem" }}>MARŻA</div>
                    <div style={{ fontFamily: "'Syne', sans-serif", fontSize: "1.5rem", fontWeight: 700, color: isPositive ? "#4ecb71" : isNegative ? "#e05555" : "#e8e4d9" }}>{formatPct(result.margin)}</div>
                    <div style={{ fontSize: "0.65rem", color: "#3a3a4e", marginTop: "0.15rem" }}>marża brutto: {formatPct(result.grossMargin)}</div>
                  </div>
                </div>
              </div>
            )}
            ...
            <button
              ...
              style={{ width: "100%", background: "linear-gradient(135deg, #4ecb71 0%, #2a9d47 100%)", border: "none", borderRadius: "8px", color: "#0f0f13", fontSize: "0.95rem", fontWeight: 600, fontFamily: "inherit", padding: "0.75rem", cursor: "pointer" }}
            >
        ...
        <div style={{ background: "#16161e", borderRadius: "16px", padding: "1.5rem", border: "1px solid #2a2a36", marginTop: "0.5rem" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1rem" }}>
            <div style={{ fontSize: "0.7rem", letterSpacing: "0.12em", color: "#5a5a6e" }}>ZAPISANE KALKULACJE ({savedCalculations.length})</div>
            {savedCalculations.length > 0 && (
              <button onClick={handleExportToExcel} style={{ background: "#f5a623", color: "#0f0f13", border: "none", borderRadius: "6px", padding: "0.4rem 0.8rem", fontSize: "0.75rem", fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>
          ...
          {savedCalculations.length === 0 ? (
            <div style={{ color: "#3a3a50", fontSize: "0.8rem", textAlign: "center", padding: "1.5rem 0" }}>Lista jest pusta. Skonfiguruj produkt i kliknij „Dodaj tę kalkulację do listy”.</div>
          ) : (
                ...
                <tbody>
                  {savedCalculations.map(item => (
                    <tr key={item.id}>
                      ...
                      <td>
                        <button onClick={() => handleRemoveFromList(item.id)} style={{ background: "transparent", border: "none", color: "#5a5a6e", cursor: "pointer", fontSize: "0.9rem" }}>✕</button>
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        <div style={{ textAlign: "center", fontSize: "0.65rem", color: "#2a2a3a", letterSpacing: "0.05em", paddingBottom: "1rem" }}>
          Kursy walut: frankfurter.app (ECB) · Repozytorium uproszczone
        </div>
      </div>
    </div>
  );
