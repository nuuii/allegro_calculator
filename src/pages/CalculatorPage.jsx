...
export default function CalculatorPage({
  ...
  edgeConfig, result, isPositive, isNegative, editingId, savedCalculations,
    ...
    <div style={{ width: '100%', maxWidth: '1140px' }}>
      <div className="workspace-grid">
        ...
        <div style={{ background: "#121218", borderRadius: "12px", padding: "1.25rem", border: "1px solid #1e1e26", display: "flex", flexDirection: "column", gap: "1rem" }}>
          ...
          <div>
            ...
            {edgeConfig.isScraperActive ? (
              <button
                type="button" onClick={handleFindCheapestOffer} disabled={eanLoading}
                style={{ width: "100%", marginTop: "0.6rem", background: "#1e1e28", border: "1px solid #f5a623", color: "#f5a623", borderRadius: "6px", padding: "0.5rem", fontSize: "0.75rem", fontWeight: 600, cursor: eanLoading ? "not-allowed" : "pointer", fontFamily: "inherit", transition: "all 0.2s" }}
              >
                {eanLoading ? <span>⟳ Trwa scrapowanie cen...</span> : "🔍 Sprawdź Allegro po EAN"}
              </button>
            ) : (
              <div style={{ marginTop: '0.6rem', padding: '0.6rem', background: '#220f0f', border: '1px solid #e05555', borderRadius: '6px', color: '#e05555', fontSize: '0.75rem', textAlign: 'center' }}>⚠️ Scraper nieaktywny.</div>
            )}
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 120px", gap: "0.75rem", alignItems: "start", marginTop: "0.75rem" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "0.3rem" }}>
                ...
                <input type="number" min="1" step="1" value={quantity} onChange={e => setQuantity(e.target.value)} style={{ width: "100%", background: "#1e1e28", border: "1px solid #2d2d3d", borderRadius: "6px", color: "#e8e4d9", fontSize: "0.95rem", fontFamily: "inherit", padding: "0.5rem", height: "42px" }} />
              </div>
            </div>
          </div>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "0.5rem", background: "#161622", padding: "0.5rem", borderRadius: "6px" }}>
            ...
            <div style={{ fontSize: "0.68rem" }}>
              {ratesLoading ? <span style={{ color: "#6a6a82" }}>⟳ API...</span> : currentRate && purchaseCurrency !== "PLN" ? <span style={{ color: "#4ecb71" }}>1 {purchaseCurrency} = {currentRate.toFixed(4)} zł</span> : <span style={{ color: "#4a4a5e" }}>Baza: PLN</span>}
            </div>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1.3fr 0.95fr 0.85fr", gap: "0.75rem", alignItems: "start" }}>
            <div>
              ...
              <div style={{ display: "flex", gap: "0.5rem", flexWrap: "wrap", alignItems: "center" }}>
                <div style={{ display: "flex", alignItems: "center", gap: "0.25rem" }}>
                  <input type="text" value={allegro} onChange={e => setAllegro(e.target.value)} style={{ width: "56px", background: "#1e1e28", border: "1px solid #2d2d3d", borderRadius: "6px", color: "#f5a623", fontSize: "0.95rem", padding: "0.5rem", textAlign: "center", fontFamily: "inherit" }} />
                  <span style={{ color: "#f5a623", fontWeight: 700 }}>%</span>
                </div>
                <div style={{ display: "flex", gap: "0.2rem", flexWrap: "wrap" }}>
                  {[5, 10, 15].map(v => <button key={v} onClick={() => setAllegro(String(v))} style={{ background: "#22222e", color: "#8a8a9e", border: "none", borderRadius: "5px", fontSize: "0.72rem", padding: "0.45rem 0.6rem", cursor: "pointer" }}>{v}%</button>)}
                </div>
              </div>
              <div style={{ display: "flex", flexWrap: "wrap", gap: "0.5rem", alignItems: "center", marginTop: "0.75rem" }}>
                <button type="button" onClick={() => setAllegroDiscounted(v => !v)} style={{ background: allegroDiscounted ? "linear-gradient(135deg, #4ecb71, #2a9d47)" : "#22222e", color: allegroDiscounted ? "#0d0d11" : "#8a8a9e", border: "none", borderRadius: "6px", fontSize: "0.75rem", padding: "0.55rem 0.85rem", cursor: "pointer", minWidth: "148px" }}>{allegroDiscounted ? "50% RABAT" : "Rabat 50%"}</button>
              </div>
            </div>
            ...
            <div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0,1fr))", gap: "0.35rem" }}>
                {vatOptions.map(v => <button key={v} onClick={() => setVat(v)} style={{ background: vat === v ? "linear-gradient(135deg, #f5a623, #f0623a)" : "#22222e", color: vat === v ? "#0d0d11" : "#8a8a9e", border: "none", borderRadius: "5px", padding: "0.55rem 0", fontSize: "0.75rem", fontWeight: 600, cursor: "pointer" }}>{v}%</button>)}
              </div>
            </div>

            <div>
              <label style={{ fontSize: "0.68rem", color: "#6a6a82", display: "block", marginBottom: "0.3rem" }}>DOSTAWA</label>
              <button type="button" onClick={() => setIncludeDelivery(v => !v)} style={{ width: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", background: "#161622", padding: "0.75rem 0.65rem", borderRadius: "8px", border: "1px solid #2d2d3d", cursor: "pointer" }}>
        ...
        <div style={{ background: "#121218", borderRadius: "12px", padding: "1.25rem", border: result ? (isPositive ? "1px solid #1a3a22" : isNegative ? "1px solid #3a1a1a" : "1px solid #1e1e26") : "1px solid #1e1e26", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
          {!result ? (
            <div style={{ color: "#4a4a5e", fontSize: "0.85rem", textAlign: "center", margin: "auto 0" }}>Wprowadź cenę oferty po lewej stronie...</div>
          ) : (
              ...
              {result.profit !== undefined && (
                <div style={{ background: isPositive ? "#0f2214" : isNegative ? "#220f0f" : "#161622", borderRadius: "8px", padding: "0.75rem 1rem", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <div>
                    ...
                    <span style={{ fontSize: "1.4rem", fontWeight: 800, color: isPositive ? "#4ecb71" : isNegative ? "#e05555" : "#e8e4d9" }}>{isPositive ? "+" : ""}{formatPLN(result.profit)}</span>
                  </div>
                  <div style={{ textAlign: "right" }}>
                    <span style={{ fontSize: "0.65rem", color: "#6a6a82", display: "block" }}>MARŻA ZWROTU</span>
                    <span style={{ fontSize: "1.2rem", fontWeight: 700, color: isPositive ? "#4ecb71" : isNegative ? "#e05555" : "#e8e4d9" }}>{formatPct(result.margin)}</span>
                  </div>
                </div>
              )}

              <div style={{ display: "flex", gap: "0.5rem" }}>
                <button onClick={handleAddToList} style={{ flex: 1, background: "linear-gradient(135deg, #4ecb71 0%, #2a9d47 100%)", border: "none", borderRadius: "6px", color: "#0d0d11", fontSize: "0.88rem", fontWeight: 600, padding: "0.6rem", cursor: "pointer", fontFamily: "inherit" }}>
                  {editingId ? "✓ AKTUALIZUJ WYCENĘ" : "＋ ZAPISZ DO LISTY ZBIORCZEJ"}
                </button>
                {editingId && <button onClick={handleCancelEdit} style={{ background: "#1e1e28", border: "1px solid #e05555", color: "#e05555", borderRadius: "6px", fontSize: "0.88rem", fontWeight: 600, padding: "0.6rem", cursor: "pointer", fontFamily: "inherit", minWidth: "100px" }}>✕ ANULUJ</button>}
              </div>
            </div>
          )}
        </div>
      ...
      {/* Rejestr wycen na samym dole pod układem siatki */}
      <ResultsTable
        savedCalculations={savedCalculations} handleExportToExcel={handleExportToExcel}
        onSaveWholeOffer={onSaveWholeOffer} handleEditItem={handleEditItem}
        handleRemoveFromList={handleRemoveFromList} formatPLN={formatPLN} formatPct={formatPct}
      />
    </div>
  );
}
